/**
 * PET Wave-1 rollback. Deletes PET STP rows for Wave-1 company_ids only.
 * Requires --write. Aborts if PCH !== 350, ENV !== 24, or INS !== 22.
 */
import { createClient, type SupabaseClient } from "@supabase/supabase-js";
import { planPetWave1Rollback } from "./pet-wave1-gates";
...
async function main() {
  ...
  const plan = planPetWave1Rollback({
  ...
  if (write && !writeError && (!pchSafe || !envSafe || !insSafe)) {
    writeError = `Protection abort after rollback: PCH ${pchAfter.count} ENV ${envAfter.count} INS ${insAfter.count}`;
  }

  console.log(
    JSON.stringify(
      {
        step: "PET Wave-1 rollback",
        writeFlag: write,
        plan,
        deleted,
        writeError,
