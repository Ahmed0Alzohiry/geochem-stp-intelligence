/**
 * Build the INS Wave-1 STP payload from the frozen 32.2.2 IDs only.
...
import { validateInsWave1Payload } from "./ins-wave1-gates";
import {
  INS_SERVICE_ID,
  INS_WAVE1_ACCOUNTS,
...
export async function buildInsWave1PersistPayload(scoredAt: string): Promise<{
  service: { id: string; name: string; service_code: string };
  payload: CompanyServiceStpScoreInsert[];
  insUniverseEligible: number;
  ...
  if (!services || services.id !== INS_SERVICE_ID) {
    throw new Error(`Live INS service_id ${services?.id ?? "missing"} !== frozen ${INS_SERVICE_ID}`);
  }

  let companySelect =
    "id, company_name, industry, subsector, customer_type, parent_company_name, is_existing_geochem_customer, account_status, city";
  let companiesRes = await supabase.from("companies").select(companySelect).in("id", [...INS_WAVE1_COMPANY_IDS]);
  if (companiesRes.error) {
    companySelect = "id, company_name, industry, subsector, customer_type, parent_company_name, account_status, city";
    companiesRes = await supabase.from("companies").select(companySelect).in("id", [...INS_WAVE1_COMPANY_IDS]);
  }
  ...
  for (const entry of INS_WAVE1_ACCOUNTS) {
    ...
    if (result.eligibility !== "ELIGIBLE") errors.push(`Not ELIGIBLE: ${entry.companyId}`);
    payload.push(
  ...
  const check = validateInsWave1Payload(payload);
  if (!check.ok) errors.push(...check.errors);
  const groups = payload.map((row) => row.account_group_key);
  if (new Set(groups).size !== groups.length) errors.push("Duplicate account_group_key in INS Wave-1 payload.");
  if (payload.length !== INS_WAVE1_EXPECTED_COUNT || errors.length > 0) {
    throw new Error(errors.join(" | ") || `INS Wave-1 payload length ${payload.length}`);
  }

  return {
    service: { id: services.id, name: services.name, service_code: services.service_code },
    payload,
    insUniverseEligible: payload.length,
