/**
 * OCM Wave-1 rollback. Deletes OCM STP rows for Wave-1 company_ids only.
 * Requires --write. Aborts if PCH !== 350, ENV !== 24, INS !== 22, or PET !== 18.
 */
import { createClient, type SupabaseClient } from "@supabase/supabase-js";
import { planOcmWave1Rollback } from "./ocm-wave1-gates";
...
async function main() {
  ...
  const plan = planOcmWave1Rollback({
  ...
  const insSafe = insAfter.count === INS_WAVE1_EXPECTED_COUNT;
  const petSafe = petAfter.count === PET_EXPECTED_CURRENT_COUNT;
  if (write && !writeError && (!pchSafe || !envSafe || !insSafe || !petSafe)) {
    writeError = `Protection abort after rollback: PCH ${pchAfter.count} ENV ${envAfter.count} INS ${insAfter.count} PET ${petAfter.count}`;
  }

  console.log(
    JSON.stringify(
      {
        step: "OCM Wave-1 rollback",
        writeFlag: write,
        plan,
        deleted,
        writeError,
