/**
 * PET Wave-1 persist/rollback gates. Pure functions — no database I/O.
 * Never accepts PCH, ENV, or INS service_ids. Persist requires --write on the PET writer.
...
import {
  ...
  petWave1CompanyIdSet,
} from "./pet-wave1-manifest";

export type PetPersistPlanAction = "insert" | "noop" | "rollback" | "abort";

export type PetWave1GateResult = {
  ok: boolean;
  action: PetPersistPlanAction;
  errors: string[];
};

export function rejectNonPetServiceId(serviceId: string): string | null {
  if (serviceId === PCH_SERVICE_ID) return "PET writer rejected PCH service_id.";
  if (serviceId === ENV_SERVICE_ID) return "PET writer rejected ENV service_id.";
  if (serviceId === INS_SERVICE_ID) return "PET writer rejected INS service_id.";
  if (serviceId !== PET_SERVICE_ID) return `PET writer rejected non-PET service_id ${serviceId}.`;
  return null;
}
...
export function validatePetWave1Payload(rows: CompanyServiceStpScoreInsert[]): PetWave1GateResult {
  const errors: string[] = [];
  const approved = petWave1CompanyIdSet();

  if (rows.length !== PET_WAVE1_EXPECTED_COUNT) {
    errors.push(`Expected PET Wave-1 count ${PET_WAVE1_EXPECTED_COUNT}, got ${rows.length}.`);
  }

  for (const row of rows) {
    const rejected = rejectNonPetServiceId(row.service_id);
    if (rejected) errors.push(rejected);
    if (!approved.has(row.company_id)) errors.push(`Unexpected PET company ${row.company_id}.`);
  ...
  if (unique.size !== payloadIds.length) errors.push("Duplicate company_id + service_id in PET payload.");
  if (payloadIds.join(",") !== PET_WAVE1_COMPANY_IDS.join(",")) {
    errors.push("PET payload order must match frozen Wave-1 rank order.");
  }

  for (const companyId of PET_WAVE1_COMPANY_IDS) {
    if (!unique.has(companyId)) errors.push(`Missing approved PET company ${companyId}.`);
  }

  const groups = rows.map((row) => row.account_group_key);
  if (new Set(groups).size !== groups.length) errors.push("Duplicate account_group_key in PET Wave-1 payload.");

  const schema = validateStpPayload(rows, PET_SERVICE_ID);
...
export function planPetWave1Persist(opts: {
  ...
  const payloadCheck = validatePetWave1Payload(opts.payload);
  if (!payloadCheck.ok) errors.push(...payloadCheck.errors);

  const petIds = [...new Set(opts.petCurrentCompanyIds)];
  const approved = petWave1CompanyIdSet();
  if (petIds.some((id) => !approved.has(id))) {
    errors.push("Existing PET current rows include unexpected companies.");
  }
  ...
  if (opts.petCurrentCount === PET_WAVE1_EXPECTED_COUNT && petIds.length === PET_WAVE1_EXPECTED_COUNT) {
    const missing = PET_WAVE1_COMPANY_IDS.filter((id) => !petIds.includes(id));
    if (missing.length === 0) {
    ...
    return {
      ok: false,
      action: "abort",
      errors: [`PET current count is ${PET_WAVE1_EXPECTED_COUNT} but missing approved ids: ${missing.join(", ")}`],
    };
  }

  if ((opts.petCurrentCount ?? 0) > 0) {
    return {
      ok: false,
      action: "abort",
      errors: [`PET current count ${opts.petCurrentCount} is not 0 or ${PET_WAVE1_EXPECTED_COUNT}; refuse to write.`],
    };
  }
...
export function planPetWave1Rollback(opts: {
  pchCurrentCount: number | null;
  envCurrentCount: number | null;
  insCurrentCount: number | null;
  petCurrentCompanyIds: string[];
  ...
  if (ins) errors.push(ins);
  const approved = petWave1CompanyIdSet();
  if (opts.petCurrentCompanyIds.some((id) => !approved.has(id))) {
    errors.push("Rollback abort: PET current rows include companies outside Wave-1.");
  }
