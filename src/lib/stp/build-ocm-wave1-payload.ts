/**
 * Build the OCM Wave-1 STP payload from the frozen 32.4.3 IDs only.
...
import { validateOcmWave1Payload } from "./ocm-wave1-gates";
...
export async function buildOcmWave1PersistPayload(scoredAt: string): Promise<{
  service: { id: string; name: string; service_code: string };
  payload: CompanyServiceStpScoreInsert[];
  ocmUniverseEligible: number;
  ...
  if (!services || services.id !== OCM_SERVICE_ID) {
    throw new Error(`Live OCM service_id ${services?.id ?? "missing"} !== frozen ${OCM_SERVICE_ID}`);
  }

  let companySelect =
    "id, company_name, industry, subsector, customer_type, parent_company_name, is_existing_geochem_customer, account_status, city";
  let companiesRes = await supabase.from("companies").select(companySelect).in("id", [...OCM_WAVE1_COMPANY_IDS]);
  if (companiesRes.error) {
    companySelect = "id, company_name, industry, subsector, customer_type, parent_company_name, account_status, city";
    companiesRes = await supabase.from("companies").select(companySelect).in("id", [...OCM_WAVE1_COMPANY_IDS]);
  }
  ...
  for (const entry of OCM_WAVE1_ACCOUNTS) {
    ...
    row.positioning_statement = positioningForUseCase("OCM", liveName, entry.useCase);
    row.targeting_reason = [
      result.targetingReason,
      `Existing GEOCHEM overlap ${entry.overlap} raises priority only and does not create OCM eligibility.`,
    ].join(" · ");
    payload.push(row);
  }

  const check = validateOcmWave1Payload(payload);
  if (!check.ok) errors.push(...check.errors);
  if (payload.length !== OCM_WAVE1_EXPECTED_COUNT || errors.length > 0) {
    throw new Error(errors.join(" | ") || `OCM Wave-1 payload length ${payload.length}`);
  }

  return {
    service: { id: services.id, name: services.name, service_code: services.service_code },
    payload,
    ocmUniverseEligible: payload.length,
