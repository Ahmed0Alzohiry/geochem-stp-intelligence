/**
 * PET Wave-1 post-write verification. SELECT only. Does not persist.
 */
...
import { PET_SERVICE_ID, PET_WAVE1_ACCOUNTS, PET_WAVE1_COMPANY_IDS, PET_WAVE1_EXPECTED_COUNT } from "./pet-wave1-manifest";
...
async function main() {
  ...
  const { data: petRows, error } = await supabase
    .from("company_service_stp_current")
    .select(
      "id, company_id, service_id, account_group_key, commercial_score, tier, application_fit, data_confidence_score, data_confidence_band, positioning_statement, ranking_eligible, scoring_model_version, scored_at",
    )
  ...
  const expected = await buildPetWave1PersistPayload(new Date().toISOString());
  const expectedById = new Map(expected.payload.map((row) => [row.company_id, row]));

  const unexpected = petIds.filter((id) => !PET_WAVE1_COMPANY_IDS.includes(id));
  ...
  const dup = petIds.length !== new Set(petIds).size;
  const groups = rows.map((row) => row.account_group_key);
  const uniqueGroups = new Set(groups).size;

  const scoreMismatches = PET_WAVE1_ACCOUNTS.filter((entry) => {
  ...
  const targeting = await getStpCurrentForService({ service: "PET", page: "1" });
  const pchTargeting = await getStpCurrentForService({ service: "PCH", page: "1" });
  const firstPet = targeting.rows[0];
  const detail = firstPet ? await getStpAccountDetail(firstPet.id, { service: "PET" }) : null;

  const pchMax = pchTimes?.[0]?.scored_at ?? null;
  const envMax = envTimes?.[0]?.scored_at ?? null;
  const insMax = insTimes?.[0]?.scored_at ?? null;
  const petMin = petTimes?.[0]?.scored_at ?? null;
  const pchNotRewritten = Boolean(pchMax && petMin && pchMax < petMin);
  const envNotRewritten = Boolean(envMax && petMin && envMax < petMin);
  const insNotRewritten = Boolean(insMax && petMin && insMax < petMin);
  ...
  const checks = {
    ...
    noDup: !dup,
    uniqueGroups18: uniqueGroups === 18,
    allPetServiceId: rows.every((row) => row.service_id === PET_SERVICE_ID),
    ...
    targetingPet18: targeting.total === 18 && targeting.readiness === "CONFIGURED",
    targetingPetOnly: targeting.rows.every((row) => row.serviceCode === "PET" && row.serviceId === PET_SERVICE_ID),
    ...
    accountDetail: Boolean(detail && detail.serviceCode === "PET" && detail.companyId === firstPet?.companyId),
  };
    ...
    JSON.stringify(
      {
        ...
        uniquePetAccounts: new Set(petIds).size,
        uniqueGroups,
        duplicates: dup,
        unexpected,
        missing,
        ...
        petMinScoredAt: petMin,
        petReadiness: serviceReadiness("PET", pet),
      },
      null,
      2,
    ),
